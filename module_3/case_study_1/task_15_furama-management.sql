...
-- Task 14.	Hiên thi thông tin tât ca các Dich vu đi kèm chi moi đuoc su dung môt lân duy nhât. 
-- Thông tin hiên thi bao gôm ma_hop_dong, ten_loai_dich_vu, ten_dich_vu_di_kem, so_lan_su_dung 
-- (đuoc tính dua trên viec count các ma_dich_vu_di_kem).
select
	hd.hdid as ma_hop_dong,
    ldv.ldvname as ten_loai_dich_vu,
    dvdk.dvdkname as ten_dich_vu_di_kem,
    count_dvdk as so_lan_su_dung
from
	hop_dong hd
...
join (select hdctdvdkid, count(hdctdvdkid) as count_dvdk from hop_dong_chi_tiet group by hdctdvdkid) temp_table on hdct.hdctdvdkid = temp_table.hdctdvdkid
where count_dvdk = 1;

-- step 1: list dich_vu_di_kem va count các ma_dich_vu_di_kem
-- select hdctdvdkid, count(hdctdvdkid) as count_dvdk from hop_dong_chi_tiet group by hdctdvdkid;