...
-- Task 12. Hiên thi thông tin ma_hop_dong, ho_ten (nhân viên), ho_ten (khách hàng), so_dien_thoai (khách hàng), ten_dich_vu, so_luong_dich_vu_di_kem 
-- (đuoc tính dua trên viec sum so_luong o dich_vu_di_kem), tien_dat_coc cua tât ca các dich vu đã tung đuoc khách hàng đăt vào 3 tháng cuôi năm 2020 
-- nhung chua tung đuoc khách hàng đat vào 6 tháng đâu năm 2021.
select
	hd.hdid as ma_hop_dong,
	nv.nvhoten as ho_ten_nhanvien,
    kh.khhoten as ho_ten_khachhang,
    kh.khsdt as so_dienthoai_khach_hang,
    dv.dvname as ten_dich_vu,
    sum(hdct.soluong) as so_luong_dich_vu_di_kem,
    hd.hdcoctruoc
from
	hop_dong hd
join nhan_vien nv on hd.hdnhanvienid = nv.nvid
join khach_hang kh on hd.hdkhachhangid = kh.khid
join dich_vu dv on hd.hddichvuid = dv.dvid
join hop_dong_chi_tiet hdct on hd.hdid = hdct.hdcthdid
join dich_vu_di_kem dvdk on hdct.hdctdvdkid = dvdk.dvdkid
where
	kh.khid in (select distinct hd.hdkhachhangid from hop_dong hd where quarter(hd.hdbegindate) = 4 and year(hd.hdbegindate) = 2020) and
    kh.khid not in (select distinct hd.hdkhachhangid from hop_dong hd where quarter(hd.hdbegindate) in (1,2) and year(hd.hdbegindate) = 2021)
group by hdct.hdcthdid, hd.hdid;
