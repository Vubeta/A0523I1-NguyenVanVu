...
select
	dv.dvid as ma_dich_vu,
    dv.dvname as ten_dich_vu,
    dv.dvarea as dien_tich,
    dv.dvmaxnumpeople as so_nguoi_toi_da,
    dv.dvprice as chi_phi_thue,
    ldv.ldvname as ten_loai_dich_vu
    from dich_vu dv
    join loai_dich_vu ldv on dv.dvloaidvid = ldv.ldvid
    where 
		dv.dvid in (select distinct hd.hddichvuid from hop_dong hd where year(hd.hdbegindate)=2020) and
		dv.dvid not in (select distinct hd.hddichvuid from hop_dong hd where year(hd.hdbegindate)=2021);