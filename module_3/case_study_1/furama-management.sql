...
-- Task 1. Thêm moi thông tin cho tat ca các bang có trong CSDL đe có the thoa mãn các yêu cau bên duoi.
...
-- Task 2. Hien thi thông tin cua tat ca nhân viên có trong he thong có tên bat đau là mot trong các ký tu “H”, “T” hoac “K” và có toi đa 15 kí tu.
select * from nhan_vien where nvhoten like "H%" or nvhoten like "T%" or nvhoten like "K%" and length(nvhoten) <=15;

-- Task 3. Hien thi thông tin cua tat ca khách hàng có đo tuoi tu 18 đen 50 tuoi và có đia chi o “Đà Nang” hoac “Quang Tri”
select * 
from khach_hang 
where ((year(current_date)-year(khngaysinh))-(right(current_date,5)<right(khngaysinh,5)) between 18 and 50) 
		and khdiachi like '%Da Nang%' or khdiachi like '%Quang Tri%';

-- Task 4. Đem xem tuong ung voi moi khách hàng đã tung đat phòng bao nhiêu lan. 
-- Ket qua hien thi đuoc sap xep tăng dan theo so lan đat phòng cua khách hàng. 
-- Chi đem nhung khách hàng nào có Tên loai khách hàng là “Diamond”.
select kh.khid, kh.khhoten, count(kh.khid) as datphong_count 
from hop_dong hd 
join khach_hang kh on kh.khid = hd.hdkhachhangid and kh.khloaikhachid = 1 
group by kh.khid
order by datphong_count;

-- Task 5. Hien thi ma_khach_hang, ho_ten, ten_loai_khach, ma_hop_dong, ten_dich_vu, ngay_lam_hop_dong, ngay_ket_thuc, tong_tien 
-- (Voi tong tien đuoc tính theo công thuc nhu sau: 
-- Chi Phí Thuê + So Luong * Giá, 
-- voi So Luong và Giá là tu bang dich_vu_di_kem, hop_dong_chi_tiet) cho tat ca các khách hàng đã tung đat phòng. 
-- (nhung khách hàng nào chua tung đat phòng cũng phai hien thi ra).
select
	kh.khid as ma_khach_hang,
	kh.khhoten as ho_ten,
	lk.lkname as ten_loai_khach,
    hd.hdid as ma_hop_dong,
    dv.dvname as ten_dich_vu,
    hd.hdbegindate as ngay_lam_hop_dong,
    hd.hdenddate as ngay_ket_thuc,
    (dv.dvprice + sum(ifnull((hdct.soluong * dvdk.dvdkprice),0))) as tong_tien
from khach_hang kh
left join loai_khach lk on kh.khloaikhachid = lk.lkid
left join hop_dong hd on hd.hdkhachhangid = kh.khid
left join dich_vu dv on hd.hddichvuid = dv.dvid
left join hop_dong_chi_tiet hdct on hd.hdid = hdct.hdcthdid
left join dich_vu_di_kem dvdk on hdct.hdctdvdkid = dvdk.dvdkid
group by kh.khid, hd.hdid, hd.hdbegindate, hd.hdenddate;
-- Task 6. Hien thi ma_dich_vu, ten_dich_vu, dien_tich, chi_phi_thue, ten_loai_dich_vu 
-- cua tat ca các loai dich vu chua tung đuoc khách hàng thuc hien đăt tu quý 1 cua năm 2021 (Quý 1 là tháng 1, 2, 3).
select
	dvid as ma_dich_vu,
    dvname as ten_dich_vu,
    dvarea as dien_tich,
    dvprice as chi_phi_thue,
    ldv.ldvname as ten_loai_dich_vu
from dich_vu dv
join loai_dich_vu ldv on dv.dvloaidvid = ldv.ldvid
where dv.dvid not in (select hd.hddichvuid from hop_dong hd where quarter(hd.hdbegindate)=1 and year(hd.hdbegindate)=2021);
...
-- Task 7. Hiên thi thông tin ma_dich_vu, ten_dich_vu, dien_tich, so_nguoi_toi_da, chi_phi_thue, ten_loai_dich_vu cua tât ca 
-- các loai dich vu đã tung đuoc khách hàng đăt phòng trong năm 2020 nhung chua tung đuoc khách hàng đăt phòng trong năm 2021.
