...
-- 13.	Hiên thi thông tin các Dich vu đi kèm đuoc su dung nhiêu nhât boi các Khách hàng đã đăt phòng. 
-- (Luu ý là có thê có nhiêu dich vu có sô lân su dung nhiêu nhu nhau).
select dvdkid, dvdkname, total_dvdk
from dich_vu_di_kem dvdk
join (select hdctdvdkid, sum(hdct.soluong) as total_dvdk from hop_dong_chi_tiet hdct group by hdct.hdctdvdkid) as dv_hdct on dvdk.dvdkid = dv_hdct.hdctdvdkid
where total_dvdk = (select max(total_dvdk) as max_dvdk from (select sum(hdct.soluong) as total_dvdk from hop_dong_chi_tiet hdct group by hdct.hdctdvdkid) as temp_table);

-- step 1: list ma_dich_vu + so_luong_dich_vu_di_kem
-- select hdctdvdkid, sum(hdct.soluong) as total_dvdk from hop_dong_chi_tiet hdct group by hdct.hdctdvdkid;
-- step 2: max so_luong_dich_vu_di_kem cua step 1
-- select max(total_dvdk) as max_dvdk from (select sum(hdct.soluong) as total_dvdk from hop_dong_chi_tiet hdct group by hdct.hdctdvdkid) as temp_table;