...
-- Task 9. Thuc hiên thông kê doanh thu theo tháng, nghĩa là tuong ung voi môi tháng trong năm 2021 thì se có bao nhiêu khách hàng thuc hiên đăt phòng.
select month(hdbegindate) as thang, count(hdkhachhangid) as so_luong_khach_hang
from hop_dong
where year(hdbegindate)=2021
group by month(hdbegindate)
order by thang;