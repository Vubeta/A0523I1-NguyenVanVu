...
-- Task 11. Hiên thi thông tin các dich vu đi kèm đã đuoc su dung boi nhung khách hàng có ten_loai_khach là “Diamond” và có dia_chi o “Vinh” hoac “Quang Ngãi”.
select dvdkid ,dvdkname
from dich_vu_di_kem dvdk 
where dvdk.dvdkid in (
	select hddichvuid 
	from hop_dong hd 
	where hdkhachhangid in (
		select khid 
		from khach_hang kh 
		where kh.khdiachi like '%Quang Ngai%' or kh.khdiachi like '%Vinh%' and 
		khloaikhachid in (
			select lkid from loai_khach lk where lkname = 'Diamond'
		)
	)
);