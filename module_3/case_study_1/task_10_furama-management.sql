...
-- Task 10. Hiên thi thông tin tuong ung voi tung hop đông thì đã su dung bao nhiêu dich vu đi kèm. 
-- Kêt qua hiên thi bao gôm ma_hop_dong, ngay_lam_hop_dong, ngay_ket_thuc, tien_dat_coc, so_luong_dich_vu_di_kem 
-- (đuoc tính dua trên viec sum so_luong o dich_vu_di_kem).
select
	hd.hdid as ma_hop_dong,
	hd.hdbegindate as ngay_lam_hop_dong,
    hd.hdenddate as ngay_ket_thuc,
    hd.hdcoctruoc as tien_dat_coc,
    sum(hdct.soluong) as so_luong_dich_vu_di_kem
from hop_dong hd
left join hop_dong_chi_tiet hdct on hd.hdid = hdct.hdcthdid
group by hd.hdid, hdct.hdcthdid
